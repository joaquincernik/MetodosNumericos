...
void gaussSeidel(double **a,double *b,double *&xnuevo,double *&xviejo,int filas,double tolerancia){
	...
	while(error>tolerancia&&iteraciones<1000){
		...
		suma=0;
		sumaParaPorcentual=0;
		for(int i=0;i<filas;i++){
			suma=suma+pow(xnuevo[i]-xviejo[i],2);
			sumaParaPorcentual=pow(xnuevo[i],2);
		}
		error= sqrt(suma);
		
		errorPorcentual=error/sqrt(sumaParaPorcentual);
	...
	printf("error=%0.20lf\n",error);
	printf("error porcentual=%0.20lf",errorPorcentual);
}
	

	
	void gaussSeidelConRElajacion(double **a,double *b,double *&xnuevo,double *&xviejo,int filas,double tolerancia,double relajacion){
		//declaracion de x
		xnuevo = (double*)malloc(filas * sizeof(double));
		xviejo = (double*)malloc(filas * sizeof(double));
		...
		while(error>tolerancia&&iteraciones<1000){
			for(int i=0;i<filas;i++){
				...
				xnuevo[i]=relajacion*xnuevo[i]+(1-relajacion)*xviejo[i];
			}
	...
	int main(int argc, char *argv[])
	{
		...
		double *xviejo= NULL;
		//jacobi
		gaussSeidel(a,b,xnuevo,xviejo,filas,0.001);
